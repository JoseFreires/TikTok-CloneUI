import { Container, TextButton } from  '../styles/loginStyles';
import HeaderLogin from '../components/HeaderLogin';
import Footer from '../components/Footer';
import { useRouter } from 'next/router';

function Login() {
    const roteamento = useRouter();
    return (
        <>
        <HeaderLogin/>
<Container>
    <h1>Entrar no TikTok</h1>
    <p>Gerencie sua conta, verifique notificações, 
        comente em vídeos e muito mais.</p>
     <ul>
         <li>
             <a onClick={()=>{
                 roteamento.push('/');
             }}>
                 <img src='https://lf16-tiktok-web.ttwstatic.com/obj/tiktok-web-us/tiktok/webapp_login/svgs/QR_Code.22d6d5db.svg' />
                 <TextButton>
                     Usar código QR
                </TextButton>
             </a>   
         </li> 
         <li>
             <a onClick={() => {
                 roteamento.push('/');
             }}>
                 <img src='data:image/png;base64,' />
                <TextButton>
                    Usar telefone/e-mail/nome de usuário
                </TextButton>
             </a>
         </li>

         <li>
             <a onClick={() => {
                     roteamento.push('/');
                 }}>
                 <img src='data:image/png;base64,' />                           
                 <TextButton>
                    Continuar com Facebook
                 </TextButton>
             </a>
         </li> 


         <li>
             <a onClick={() => {
                     roteamento.push('/');
                 }}>
                 <img src='data:image/png;base64,'/>
                 <TextButton>
                    Continuar com Google
                 </TextButton>
             </a>
         </li>  

          <li>
             <a onClick={() => {
                         roteamento.push('/');
                     }}>
                 <img src='data:image/png;base64,'/>
                 <TextButton>
                    Continuar com Twitter
                 </TextButton>
            </a>
         </li>

         <li>
             <a onClick={() => {
                         roteamento.push('/');
                     }}>
                 <img src='https://lf16-tiktok-web.ttwstatic.com/obj/tiktok-web-us/tiktok/webapp_login/svgs/apple.fcc569e4.svg'/>
                 <TextButton>
                    Continuar com Apple
                 </TextButton>
            </a>
         </li>

         <li>
             <a onClick={() => {
                         roteamento.push('/');
                     }}>
                 <img src='data:image/png;base64,'/>
                
                <TextButton> 
                    Continuar com Instagram
                </TextButton> 
             </a>
         </li>
         
    </ul>
</Container>
<Footer/>
        </>
        
    )
}

export default Login;